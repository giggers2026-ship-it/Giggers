...
exports.listJobTasks = listJobTasks;
exports.saveJobTasks = saveJobTasks;
exports.updateTask = updateTask;
exports.deleteTask = deleteTask;
exports.getCompletions = getCompletions;
exports.tickCompletion = tickCompletion;
exports.submitFormCompletion = submitFormCompletion;
exports.submitImageCompletion = submitImageCompletion;
exports.reviewCompletion = reviewCompletion;
exports.employerForceComplete = employerForceComplete;
exports.employerReopenTask = employerReopenTask;
exports.getCompletionImageUrl = getCompletionImageUrl;
 ...
 * very next read. */
function clockWindowFor(task, job, completion) {
    ...
    if (completion?.manually_reopened_at) {
        ...
        return { opensAt: reopenedAt, deadline: new Date(reopenedAt.getTime() + task.open_minutes_after * 60000) };
    }
    if (!job.date)
        return null;
    ...
    if (Number.isNaN(anchor.getTime()))
        return null;
    const opensAt = new Date(anchor.getTime() - task.open_minutes_before * 60000);
    const deadline = new Date(anchor.getTime() + task.open_minutes_after * 60000);
    return { opensAt, deadline };
}
async function mapCompletion(row, clockWindow) {
    const imageUrl = await (0, storage_1.getSignedTaskImageUrl)(row.image_path);
    return {
        id: row.id,
...
async function applyAutoFail(completions, tasksById, job) {
    const now = Date.now();
    const toFail = [];
    for (const c of completions) {
        if (c.status !== 'in_progress' && c.status !== 'submitted')
            continue;
        ...
        if (clockWindow) {
            ...
            if (!c.available_at)
                continue;
            deadline = new Date(c.available_at).getTime() + task.auto_fail_minutes * 60000;
        }
        if (now > deadline) {
            c.status = 'failed';
            toFail.push(c.id);
    ...
    if (toFail.length > 0) {
        await supabase_1.supabase.from('application_task_completions').update({ status: 'failed' }).in('id', toFail);
    }
}
...
async function wakeClockAnchoredTasks(completions, tasks, job) {
    ...
    for (let i = 0; i < sortedTasks.length; i++) {
        ...
        if (!clockWindow || Date.now() < clockWindow.opensAt.getTime())
            continue;
    ...
    if (toWake.length > 0) {
        await supabase_1.supabase.from('application_task_completions').update({ status: 'in_progress', available_at: now }).in('id', toWake);
    }
}
...
const taskInputSchema = zod_1.z.object({
    kind: zod_1.z.enum(['opening', 'task', 'closing']),
    title: zod_1.z.string().min(1),
    description: zod_1.z.string().optional().default(''),
    completionType: zod_1.z.enum(['image', 'form', 'tick']),
    formSchema: zod_1.z.array(zod_1.z.object({ label: zod_1.z.string(), type: zod_1.z.enum(['text', 'number', 'select']), options: zod_1.z.array(zod_1.z.string()).optional() })).optional(),
    responseWindowMinutes: zod_1.z.number().int().positive().optional().default(5),
    autoFailMinutes: zod_1.z.number().int().positive().optional().default(10),
    openMinutesBefore: zod_1.z.number().int().nonnegative().optional().default(10),
    openMinutesAfter: zod_1.z.number().int().nonnegative().optional().default(10),
    anchorTime: zod_1.z.string().regex(/^\d{2}:\d{2}$/).optional(),
    requiresReview: zod_1.z.boolean().optional().default(true),
});
const saveTasksSchema = zod_1.z.object({
    tasks: zod_1.z.array(taskInputSchema).min(2), // at minimum an opening + closing task
});
// POST /api/pipeline/jobs/:jobId/tasks — replaces the full task list for a job
async function saveJobTasks(req, res) {
    const { jobId } = req.params;
    ...
    const { data: job } = await supabase_1.supabase.from('jobs').select('id, employer_id').eq('id', jobId).single();
    if (!job) {
    ...
    await supabase_1.supabase.from('job_tasks').delete().eq('job_id', jobId);
    ...
    const { data: inserted, error } = await supabase_1.supabase.from('job_tasks').insert(rows).select('*').order('sort_order', { ascending: true });
    if (error) {
        res.status(500).json({ error: error.message });
        return;
    }
...
async function updateTask(req, res) {
    ...
    const { data: task } = await supabase_1.supabase.from('job_tasks').select('*, jobs!inner(employer_id)').eq('id', taskId).single();
    if (!task) {
        res.status(404).json({ error: 'Task not found' });
        return;
    }
    if (task.jobs.employer_id !== req.user.id) {
        res.status(403).json({ error: 'Only the job owner can edit its pipeline' });
        return;
    }
    const updates = {};
    ...
    if (d.completionType !== undefined)
        updates.completion_type = d.completionType;
    if (d.formSchema !== undefined)
        updates.form_schema = d.formSchema;
    if (d.responseWindowMinutes !== undefined)
        updates.response_window_minutes = d.responseWindowMinutes;
    if (d.autoFailMinutes !== undefined)
        updates.auto_fail_minutes = d.autoFailMinutes;
    if (d.openMinutesBefore !== undefined)
        updates.open_minutes_before = d.openMinutesBefore;
    if (d.openMinutesAfter !== undefined)
        updates.open_minutes_after = d.openMinutesAfter;
    if (d.anchorTime !== undefined)
        updates.anchor_time = d.anchorTime || null;
    if (d.requiresReview !== undefined)
        updates.requires_review = d.requiresReview;
    const { data: updated, error } = await supabase_1.supabase.from('job_tasks').update(updates).eq('id', taskId).select('*').single();
    if (error || !updated) {
...
async function deleteTask(req, res) {
    ...
    await supabase_1.supabase.from('job_tasks').delete().eq('id', taskId);
    res.json({ success: true });
}
async function authorizeCompletionAccess(req, applicationId) {
    ...
    const isEmployer = application.jobs.employer_id === userId;
    if (isWorker || isEmployer)
        return { ok: true, isWorker, isEmployer };
    // Read-only clients: allowed if they've been invited to this specific job.
    if (req.user.role === 'client') {
        const { data: invite } = await supabase_1.supabase
            .from('job_clients')
            .select('id')
            ...
            .maybeSingle();
        if (invite)
            return { ok: true, isWorker: false, isEmployer: false };
    }
    return { ok: false, isWorker: false, isEmployer: false };
}
...
async function getCompletions(req, res) {
    ...
    const { data: applicationRow } = await supabase_1.supabase.from('applications').select('job_id').eq('id', applicationId).single();
    const jobId = applicationRow?.job_id;
    if (!jobId) {
        res.status(404).json({ error: 'Application not found' });
        return;
    }
    const { data: job } = await supabase_1.supabase.from('jobs').select('date, reporting_time, end_time').eq('id', jobId).single();
    await seedLegacyTasksIfMissing(jobId);
    const { data: tasks } = await supabase_1.supabase.from('job_tasks').select('*').eq('job_id', jobId).order('sort_order', { ascending: true });
    if (!tasks || tasks.length === 0) {
        res.json({ tasks: [], completions: [] });
        return;
    }
    let { data: completions } = await supabase_1.supabase
    ...
    if (missing.length > 0) {
        ...
        const { data: created } = await supabase_1.supabase.from('application_task_completions').insert(newRows).select('*');
...
async function checkClockWindowOpen(completion) {
    const { data: task } = await supabase_1.supabase.from('job_tasks').select('*').eq('id', completion.job_task_id).single();
    if (!task)
        return null;
    const { data: application } = await supabase_1.supabase.from('applications').select('job_id').eq('id', completion.application_id).single();
    if (!application)
        return null;
    const { data: job } = await supabase_1.supabase.from('jobs').select('date, reporting_time, end_time').eq('id', application.job_id).single();
    if (!job)
        return null;
...
async function checkAndCompleteApplication(applicationId) {
    const { data: application } = await supabase_1.supabase.from('applications').select('status, job_id').eq('id', applicationId).single();
    if (!application || application.status === 'completed')
        return;
    const { data: tasks } = await supabase_1.supabase.from('job_tasks').select('id').eq('job_id', application.job_id);
    if (!tasks || tasks.length === 0)
        return;
    const { data: completions } = await supabase_1.supabase
        .from('application_task_completions')
    ...
    if (!allComplete)
        return;
    await supabase_1.supabase.from('applications').update({ status: 'completed', updated_at: new Date().toISOString() }).eq('id', applicationId);
}
async function advanceNextTask(applicationId, completedTaskId) {
    const { data: application } = await supabase_1.supabase.from('applications').select('job_id').eq('id', applicationId).single();
    if (!application)
        return;
    const { data: tasks } = await supabase_1.supabase.from('job_tasks').select('*').eq('job_id', application.job_id).order('sort_order', { ascending: true });
    if (!tasks)
        return;
...
async function tickCompletion(req, res) {
    const { completionId } = req.params;
    const { data: completion } = await supabase_1.supabase.from('application_task_completions').select('*').eq('id', completionId).single();
    if (!completion) {
...
async function submitFormCompletion(req, res) {
    const { completionId } = req.params;
    const parsed = zod_1.z.object({ formData: zod_1.z.record(zod_1.z.union([zod_1.z.string(), zod_1.z.number()])) }).safeParse(req.body);
    if (!parsed.success) {
        res.status(400).json({ error: parsed.error.errors[0].message });
        return;
    }
    ...
    const { data: task } = await supabase_1.supabase.from('job_tasks').select('requires_review').eq('id', completion.job_task_id).single();
...
async function submitImageCompletion(req, res) {
    const { completionId } = req.params;
    const parsed = zod_1.z.object({ image: zod_1.z.string().min(32) }).safeParse(req.body);
    if (!parsed.success) {
        res.status(400).json({ error: parsed.error.errors[0].message });
        return;
    }
    ...
    let imagePath;
    try {
        imagePath = await (0, storage_1.uploadTaskImage)(parsed.data.image, req.user.id, `task-${completion.job_task_id}`);
    }
    catch (err) {
...
async function reviewCompletion(req, res) {
    const { completionId } = req.params;
    const parsed = zod_1.z.object({ approve: zod_1.z.boolean(), rejectionReason: zod_1.z.string().optional() }).safeParse(req.body);
    if (!parsed.success) {
        res.status(400).json({ error: parsed.error.errors[0].message });
        return;
    }
...
async function employerForceComplete(req, res) {
    const { completionId } = req.params;
    const { data: completion } = await supabase_1.supabase.from('application_task_completions').select('*').eq('id', completionId).single();
    if (!completion) {
        res.status(404).json({ error: 'Task completion not found' });
...
async function employerReopenTask(req, res) {
    const { completionId } = req.params;
    const { data: completion } = await supabase_1.supabase.from('application_task_completions').select('*').eq('id', completionId).single();
    if (!completion) {
        res.status(404).json({ error: 'Task completion not found' });
    ...
    const { data: application } = await supabase_1.supabase.from('applications').select('job_id').eq('id', completion.application_id).single();
    const { data: task } = await supabase_1.supabase.from('job_tasks').select('*').eq('id', completion.job_task_id).single();
    const { data: job } = application ? await supabase_1.supabase.from('jobs').select('date, reporting_time, end_time').eq('id', application.job_id).single() : { data: null };
...
async function getCompletionImageUrl(req, res) {
    const { completionId } = req.params;
    const { data: completion } = await supabase_1.supabase.from('application_task_completions').select('*').eq('id', completionId).single();
    if (!completion) {
        res.status(404).json({ error: 'Task completion not found' });
    ...
    const url = await (0, storage_1.getSignedTaskImageUrl)(completion.image_path);
