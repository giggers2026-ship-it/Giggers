...
exports.myClientJobs = myClientJobs;
exports.revokeClient = revokeClient;
...
async function inviteClient(req, res) {
    ...
    const { data: existing } = await supabase_1.supabase.from('job_clients').select('id').eq('job_id', jobId).eq('phone', phone).maybeSingle();
    const { data: jobClient, error } = existing
        ? await supabase_1.supabase.from('job_clients').update({ name, invite_token: inviteToken }).eq('id', existing.id).select('*').single()
        : await supabase_1.supabase.from('job_clients').insert({ job_id: jobId, employer_id: req.user.id, name, phone, invite_token: inviteToken }).select('*').single();
    if (error || !jobClient) {
    ...
    const { data: existingProfile } = await supabase_1.supabase.from('profiles').select('id').eq('phone', phone).eq('role', 'client').maybeSingle();
    if (!existingProfile) {
        const { error: profileError } = await supabase_1.supabase.from('profiles').insert({ phone, name, role: 'client', is_approved: true, is_verified: true });
        if (profileError) {
...
async function redeemInvite(req, res) {
    const parsed = zod_1.z.object({ inviteToken: zod_1.z.string().min(1) }).safeParse(req.body);
    if (!parsed.success) {
        res.status(400).json({ error: 'Missing invite token' });
        return;
    }
    const { data: jobClient } = await supabase_1.supabase.from('job_clients').select('*').eq('invite_token', parsed.data.inviteToken).single();
    if (!jobClient) {
        res.status(404).json({ error: 'Invalid or expired invite link' });
        return;
    }
    const { data: profile } = await supabase_1.supabase.from('profiles').select('*').eq('phone', jobClient.phone).eq('role', 'client').single();
    if (!profile) {
        res.status(404).json({ error: 'Client profile not found' });
        return;
    }
    await supabase_1.supabase.from('job_clients').update({ last_viewed_at: new Date().toISOString() }).eq('id', jobClient.id);
    const payload = { id: profile.id, phone: profile.phone, role: 'client', name: profile.name };
...
async function myClientJobs(req, res) {
    if (req.user.role !== 'client') {
        res.status(403).json({ error: 'Client role required' });
        return;
    }
    const { data: invites } = await supabase_1.supabase
        .from('job_clients')
...
async function revokeClient(req, res) {
    ...
    const { data: jobClient } = await supabase_1.supabase.from('job_clients').select('employer_id').eq('id', jobClientId).single();
    if (!jobClient) {
    ...
    await supabase_1.supabase.from('job_clients').delete().eq('id', jobClientId);
