...
export default function PostJob() {
  ...
  const { wallet, payForJob, addMoney } = useWalletStore();
  ...
  const executePostJob = async () => {
    payForJob(totalCost, platformFee, title);
    await postJob({
  ...
  const handleAddFunds = () => {
    const deficit = totalPayable - (wallet?.currentBalance || 0);
    addMoney(deficit);
    addToast(`₹${deficit} added to wallet successfully`, 'success');
